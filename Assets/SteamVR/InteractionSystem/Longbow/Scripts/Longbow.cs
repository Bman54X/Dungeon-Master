...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Longbow : MonoBehaviour
    {
     ...
     //   public Transform nockTransform2;
        public Transform[] nockRestTransform;
      //  public Transform nockRestTransform2;

            //stuff for shooting more than one arrow
        public Vector3[] nockToarrowHand;
        public Vector3 arrowHandPosition;
        public Vector3 arrowNockTransformToHeadset;
        public float pullLerp;
        public float lerp;
        ...
        private void HandAttachedUpdate(Hand hand)
        {
            ...
            if (nocked)
            {
               ...
               {
                    nockToarrowHand[i] = (arrowHand.arrowNockTransform[i].parent.position - nockRestTransform[0].position); // Vector from bow nock transform to arrowhand nock transform - used to align bow when drawing
                                                                       //  Vector3 nockToarrowHand2 = (arrowHand.arrowNockTransform2.parent.position - nockRestTransform2.position);
                                                                                                                                                                                     //   }
                }                                                                                                                                                              // Align bow
                                                                                                                                                                                 // Time lerp value used for ramping into drawn bow orientation
                lerp = Util.RemapNumberClamped(Time.time, nockLerpStartTime, (nockLerpStartTime + lerpDuration), 0f, 1f);
                for (int i = 0; i < NumOfArrowsToShoot; i++)
                {
                    pullLerp = Util.RemapNumberClamped(nockToarrowHand[i].magnitude, minPull, maxPull, 0f, 1f); // Normalized current state of bow draw 0 - 1
                }
                //  float pullLerp2 = Util.RemapNumberClamped(nockToarrowHand2.magnitude, minPull, maxPull, 0f, 1f);
                for (int i = 0; i < NumOfArrowsToShoot; i++)
                {
                    arrowNockTransformToHeadset = ((Player.instance.hmdTransform.position + (Vector3.down * 0.05f)) - arrowHand.arrowNockTransform[i].parent.position).normalized;
                    arrowHandPosition = (arrowHand.arrowNockTransform[i].parent.position + ((arrowNockTransformToHeadset * drawOffset) * pullLerp)); // Use this line to lerp arrowHand nock position
                }
                //   Vector3 arrowNockTransformToHeadset2 = ((Player.instance.hmdTransform.position + (Vector3.down * 0.05f)) - arrowHand.arrowNockTransform2.parent.position).normalized;
                //  Vector3 arrowHandPosition2 = (arrowHand.arrowNockTransform2.parent.position + ((arrowNockTransformToHeadset2 * drawOffset) * pullLerp));                                                                                                                            //Vector3 arrowHandPosition = arrowHand.arrowNockTransform.position; // Use this line if we don't want to lerp arrowHand nock position
                ...
                for (int i = 0; i < NumOfArrowsToShoot; i++)
                {
                    if (Vector3.Dot(nockToarrowHand[i], -nockTransform[i].forward) > 0)
                    {
                        float distanceToarrowHand = nockToarrowHand[1].magnitude * lerp;

                        nockTransform[i].localPosition = new Vector3(0f, 0f, Mathf.Clamp(-distanceToarrowHand, -maxPull, 0f));

                        nockDistanceTravelled = -nockTransform[i].localPosition.z;
                    ...
                    {
                        nockTransform[i].localPosition = new Vector3(0f, 0f, 0f);
            ...
            else
            {
                ...
                {
                    lerp = Util.RemapNumber(Time.time, lerpStartTime, lerpStartTime + lerpDuration, 0, 1);
        ...
        public void StartRotationLerp()
        {
            ...
            for (int i = 0; i < NumOfArrowsToShoot; i++)
            {
                Util.ResetTransform(nockTransform[i]);
            }
